...
|--------|----------|------------------|
| [git-object-fetching.md](git-object-fetching.md) | Can we serve reads from a partial clone without implicit fetches? | A `cat-file --batch` session with `GIT_NO_LAZY_FETCH` **fatally exits** on a missing promisor object, so the provider must be the residency authority. |
...
| [linux-fuse.md](linux-fuse.md) | Is the FUSE callback model viable? | Callback logic is implemented and tested without libfuse; a real kernel mount needs the libfuse adapter plus a privileged runner. |
| [macos-fskit.md](../future-platforms/feasibility-macos-fskit.md) | FSKit viability? | Not testable in this environment; scaffold and requirements recorded. |
| [windows-projfs.md](../future-platforms/feasibility-windows-projfs.md) | ProjFS viability? | Distinct architecture, not testable here. The `autocrlf=true` system default already surfaced via CI. |
