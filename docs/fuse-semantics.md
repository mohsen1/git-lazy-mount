...
These ops are unimplemented today and would be additive:

- **Advisory file locking** (`getlk`/`setlk`/`flock`) and **`lseek`
  `SEEK_DATA`/`SEEK_HOLE`** over the overlay FD — useful for some editors and
  sparse-file tooling, but no current workflow requires them.
- **`fallocate` `FALLOC_FL_PUNCH_HOLE`** and **`copy_file_range`** reflink within
  the overlay — bandwidth optimizations, not correctness.
- **xattr** passthrough — would need an explicit store; today these report
  `ENOSYS`, which is honest (no silent drop).
- **`RENAME_EXCHANGE`** as an atomic two-node overlay swap, instead of the
  current `ENOSYS`.

---

## 6. Considered / not built: the parent-indexed namespace store

An earlier design proposed a dedicated SQLite/WAL `namespace` crate — a
persistent, parent-indexed table (`ns_node(parent_ino, name, kind, …)`) as the
authority for overlay structure, with `children(parent)`, `has_children`, subtree
rename, and case-collision answered in O(direct children). It also proposed
extending `InodeEntry` with an `InodeSource` enum, `open_handles`, `nlink`,
`kind`, and `deleted_but_open`, plus a reserved `GITFILE_INO`.

**None of that was built, and it is recorded here only as rationale.** The
shipped overlay structure is one atomic JSON sidecar per entry under `meta/`,
keyed by `sha256(path)`, with content in native files under `content/`
(`crates/worktree/src/overlay.rs`); the in-memory index is a disposable cache
rebuilt from the sidecars on open. The inode table is the minimal `{path,
lookups, generation}` of §2, handle tracking lives in the `Handle` enum of §3,
and `.git` protection is `Projection::child_path`, not a reserved inode.

The durable value the SQLite form was meant to add — O(direct children) listing
and rename, and survival of empty dirs / tombstones across remount — the shipped
design already achieves: `Projection::readdir` merges the baseline tree with
only the direct overlay children (O(direct children), no blob fetch), and the
sidecars persist tombstones, empty dirs, and base-refs. A single transactional
log/DB remains a possible future refinement (the overlay module doc names it as
such), chiefly to consolidate per-entry fsyncs — not because the current
structure is incorrect. See [worktree-model.md](worktree-model.md) for the
overlay model and [durability-security.md](durability-security.md) for the
atomic-sidecar durability primitive.
