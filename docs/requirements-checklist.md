...
Living tracker of what's built and proven (against the [spec](design.md)). Items move here as they pass
through a real mount in CI. `[ ]` = not done, `[~]` = in progress / partial,
`[x]` = done + tested. Nothing is checked until a mounted test proves it.
...
- [x] **H** FSMonitor zero-blob first status: the first clean `git status`/`git diff` faults **0** blobs (`first_status_faults_zero_blobs_and_surfaces_edits`). The earlier "unachievable" reading was a bootstrap-ordering bug, not a fundamental limit: a freshly `read-tree`'d index carries no FSMonitor extension, so git's "mark all entries valid" pass never runs and it stats (and faults) every entry. Fix: pre-seed the FSMonitor index extension at mount (right after `read-tree`) via `AdminRepo::seed_fsmonitor_valid`, marking every entry `CE_FSMONITOR_VALID` with the journal's seq-0 token; git's `refresh_cache_ent` then early-returns before any `lstat`. Paths under a checkout conversion (filter/ident/working-tree-encoding/CRLF eol) are excluded from the seed so git checks them normally. Verified zero-fault on an 81k-file real mount. FSMonitor *is* wired (`fsmonitor` test) for change detection + faster repeat status. See limitations P1/R6.
- [x] **I** large-file I/O: 4 MiB in-place 4 KiB writes (`m2_semantics`) plus a 64 MiB read that grows daemon RSS ~2 MiB, not 64 MiB (`large_file`). Streamed, bounded memory.
...
- [x] 17 rebase abort restores state. *Real mount (`git_extra`).* The `--continue` flow was verified correct through a real mount.
...
- [x] clean `git status`: both the *first* and **repeat** clean status fetch 0 blobs (measured, `status_hydration`; first-status zero-fault covered by `first_status_faults_zero_blobs_and_surfaces_edits`). The mount pre-seeds the FSMonitor index extension right after `read-tree` (`AdminRepo::seed_fsmonitor_valid`, marking every entry `CE_FSMONITOR_VALID` with the seq-0 token), so git's `refresh_cache_ent` early-returns before any `lstat` and no blob size is hydrated. Paths under a checkout conversion are excluded from the seed so git checks them normally. (`ls -l`/`stat` still faults a blob's size once via `getattr`; that R6 cost is separate from `git status`.)
