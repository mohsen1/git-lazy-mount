...
- [✗] **H** FSMonitor zero-blob first status: unachievable with stock git + `blob:none`. Git must populate the index stat (incl. size) to skip the content check, the size requires fetching the blob, and `mark_fsmonitor_invalid` overrides the fsmonitor-valid bit on empty-stat entries (verified via `GIT_TRACE_FSMONITOR`). FSMonitor *is* wired (`fsmonitor` test) for change detection + faster repeat status. See limitations P1/R6.
