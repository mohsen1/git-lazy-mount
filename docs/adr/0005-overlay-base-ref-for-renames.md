...
* A clean rename is `O(1)` metadata, offline-capable, and writes zero content
  bytes. This is verified by `overlay::base_ref_stores_no_content` (no content file
  is created, and `(oid, mode)` round-trips across reopen) and the workspace rename
  tests.
