...
matches what a real `git checkout` would write: CRLF/`eol`, plus any configured
...
base revisions, so reading attributes from the store's `HEAD` would apply the wrong
filtering rules.
...
* Because filtering can change byte length (CRLF), exact `stat` size is filter- and
  context-dependent. That bears on the Windows ProjFS ContentID and placeholder
  size requirements (see [../design/future-platforms/windows.md](../future-platforms/windows.md)).
