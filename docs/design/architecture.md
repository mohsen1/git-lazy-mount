...
alias, env, or `git lazy-mount` workflow verb** (§1, §2). `git lazy-mount`
...
add|commit|switch|push|git --` means transparency has failed (§1).

## Two sources of truth (§7, §8, §25)
...
## Working-tree model (§8) — baseline + overlay
...
is known to have updated the working tree (§8.2); index-only ops (`reset
...
is protected from unlink/rename/replace/chmod/write/mkdir-beneath (§6).

## Startup as an idempotent transaction (§10)
...
states (§4.1): `creating cloning initializing-git building-index starting-daemon
...
- Stable inode table (generation, namespace identity, link/handle/lookup counts,
  ...
  not path lookups.
- `readdir` returns names + d_type only — **never** sizes or blob reads (§4.5).
...
## FSMonitor v2 (§12) + hook chaining (§13)
...
hooks (§13.1), never replacing them.

## Crate structure (§41) — built behind the transparent design
...
**superseded** (§4) and removed as the transparent path replaces them.

## Milestone plan (§42) — Linux only, real-mount tested in CI

- **M0** this: architecture + requirements checklist + Experiment A–C vertical
...
- **M6** large-repo index strategy chosen from measurements (Profiles A–D, §11).
- **M7** optional shared object cache. **M8** other platforms (out of scope here).

Priority order (§45): stock-Git correctness → user-data durability → filesystem
