...
| # | Area | Status |
|---|------|--------|
| P1 | **FSMonitor v2**: **wired** to `core.fsmonitor` (`git-lazy-mount-fsmonitor` reads the durable change journal, and every worktree mutation is recorded synchronously). Gives correct change detection (no false negatives, `fsmonitor` test) and skips the redundant stat scan on *subsequent* statuses, which is the real win on huge repos. The first-status zero-blob bootstrap is **fundamentally unachievable** with stock git + `blob:none` (verified via `GIT_TRACE_FSMONITOR`). Git marks each read-tree'd entry clean from the hook's empty reply, then calls `mark_fsmonitor_invalid` because the entry has no stat data: git must populate the stat (incl. size) to skip the content check, and under `blob:none` the size requires fetching the blob. The fsmonitor-valid bit does not override an empty-stat entry. So the *first* clean status faults each blob once (root cause is R6). | wired; first-status eager is fundamental |
