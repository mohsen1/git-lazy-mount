...
claim anything we haven't proven. For what **does** work per command, see the
compatibility matrix in [`compatibility.md`](compatibility.md); the area docs in
[`README.md`](README.md) own the depth. This page lists the refinements that
shipped, the costs that are fundamental to lazy-blob fetching (the default
`tree:0` filter), and the few items still deferred, each with the test that will
be un-`#[ignore]`d when it lands.
...
| R6 | **`getattr` size hydration (fundamental to lazy-blob fetching)**: the exact size of an unmaterialized blob requires faulting the object. Trees carry no sizes, and `cat-file --batch-check` fetches the whole promisor object to report one. So `ls -l`/`stat` of an unmaterialized file faults its blob once for the size. This is inherent to any lazy-blob filter (`tree:0`, `blob:none`). (`git status`/`git diff` do **not** fault: the seeded FSMonitor extension lets git skip the stat entirely, see P1.) Not closeable without a server-side size manifest. | by-design (fundamental) | n/a |
...
| # | Area | Status |
|---|------|--------|
| S1 | **Large-file bounded memory** (proven, `large_file`): reading a 64 MiB baseline blob grows daemon RSS far less than the file size — the test pins RSS growth under 24 MiB for a 64 MiB blob — via streamed `cat-file`→cache + `pread`. The multi-GiB / 100k-file extreme scale is heavier CI stress. The streaming and O(direct children) properties are structural and proven at representative sizes (64 MiB file, 1500-file readdir). | ✅ proven (representative) |
