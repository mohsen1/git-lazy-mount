...
## 4. Considered / not built (possible future)

The following were weighed during design and are recorded here as rationale, not
as shipped machinery. None of these types, crates, or selection gates exist in the
code.

- **Dynamic per-path skip-worktree on the mount.** Tracking *materialization*
  with the skip bit (set on first writable open, cleared on conflict stages) was
  considered as a way to let ref-moving commands skip unmaterialized paths and
  avoid materializing on `switch`. It was not built: skip-worktree semantics are
  an implementation detail of sparse-checkout, vary across Git versions, and would
  need per-Git-version compatibility gating. (Note: the *interop bridge* in §2
  does use universal skip-worktree, but only for its throwaway off-mount index.)

- **Sparse index.** Collapsing untouched directories into single
  sparse-directory entries (`mode=040000`, oid = the tree) would turn the `O(N)`
  index into `O(touched paths + boundary dirs)` on a monorepo, while the FUSE
  projection still exposes every collapsed path. Not built; the cost of expanding
  on force-full-index commands and the namespace/index mismatch were the open
  risks.

- **A virtual-working-tree provider extension to Git.** If unmodified Git proved
  too eager on branch transitions, a minimal, explicitly-advertised, upstreamable
  extension (`declare` / `update_baseline` / `materialize` / `changed`) could make
  `git switch` move the baseline without writing clean blobs. This is the
  last-resort, highest-cost path because it requires a Git change; it was not
  pursued. A wrapper/alias front-end (`git lazy-mount git --`) was explicitly
  rejected — the product stays transparent to plain `git`.

- **`index.version=4` / `feature.manyFiles` / `core.untrackedCache` /
  split-index / preload-index.** These were candidate large-`N` tunings to
  evaluate per repo. None is currently set by the mount.

These remain reasoning, not roadmap commitments. The shipped design — full index
via `git read-tree HEAD`, the FSMonitor pre-seed, and the durable change journal —
keeps the common path (clean `status`/`diff`) zero-blob and branch transitions
delta-bounded without any of the above.
